...
from datacollection.models import file_score, collectlog

def _generate_code():
    #return binascii.hexlify(os.urandom(3))
    return str(random.randint(0,10000)).zfill(4)
...
class EmailAbstractUser(AbstractBaseUser, PermissionsMixin):
    ...
    """
    username = models.CharField(_('user name'), max_length=30, unique=True, blank=False)
    ...
    # The field that required to create a superuser
...
def send_multi_format_email(template_prefix, template_ctxt, target_email):
        ...
        print ("I am sending an email to %s"%(html_file))
...
class MyUser(EmailAbstractUser):
    faculty = models.CharField('faculty', max_length=30, null=True,
        choices=FACULTY_CHOICES, blank=False, unique=False)
    macid = models.CharField('macid', max_length=30, blank=True, 
        unique=True, null=True)
    lable = models.CharField('lable', max_length=5, blank=True,
        unique=False, null=True)
    score = models.FloatField('Score', default=0.0, blank=True,
        unique=False)
